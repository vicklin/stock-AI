...
public class SpringNamedSqlGenerator implements SqlGenerator {
    ...
    public String getInsertSql() {
        ...
        List<String> insertPlaceholderColumns = new ArrayList<String>(getColumns().size());
    ...
    public String getLogicDeleteByPkSql() {
        if (isMultiPrimaryKey()) {
            return getLogicDeleteByMultiPkSql();
        } else if (isSinglePrimaryKey()) {
            return getLogicDeleteBySinglePkSql();
        }
        throw new IllegalStateException("not found primary key config on table:" + table.getTableName());
    }

    private String getLogicDeleteBySinglePkSql() {
        checkIsSinglePrimaryKey();

        StringBuilder sb = new StringBuilder("UPDATE ").append(getTableName()).append(" SET ");

        sb.append(" isDeleted = " + BaseEntity.ISDELETED_YES_VALUE);
        sb.append(" WHERE ");
    ...
    private String getLogicDeleteByMultiPkSql() {
